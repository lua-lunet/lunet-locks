    ...
    while :; do
        ...
        if request_lines 28101 "$(printf "$template" "$mid")" '"accepted":true' 40; then
            ...
            test $(( t_done - t_attempt )) -lt 30000 || {
                echo "package verify: $label ack took $((t_done - t_attempt))ms; at the leader's" >&2
                echo "30s era-poll timeout this is the timeout acknowledgment, not a committed era" >&2
                return 1
            }
            printf '%s\n' "$t_done" >"$work/$label.done"
            echo "package verify: live-reconfig $label accepted on attempt $attempts after $((t_done - t_start))ms"
            return 0
        fi
        sleep 4
    done
    echo "package verify: live-reconfig $label was refused by every drive within the retry budget" >&2
    return 1
}
...
if grep -q "STREAM ERROR" "$work/stream.err"; then
    echo "package verify: the lock stream errored across the live reconfiguration (observational):" >&2
    cat "$work/stream.err" >&2
fi
if ! test -s "$work/stream.out"; then
    echo "package verify: the lock stream produced no replies (observational)" >&2
fi
...
echo "package verify: live-reconfig stream [join window]: requests=$1 max_latency=${2}ms"
set -- $(window "$work/stream.out" "$t_inc_start" "$t_dec_start")
echo "package verify: live-reconfig stream [increment window]: requests=$1 max_latency=${2}ms"
set -- $(window "$work/stream.out" "$t_dec_start" "$t_leave_start")
echo "package verify: live-reconfig stream [decrement window]: requests=$1 max_latency=${2}ms"
set -- $(window "$work/stream.out" "$t_leave_start" "$t_stream_end")
echo "package verify: live-reconfig stream [leave window]: requests=$1 max_latency=${2}ms"
set -- $(window "$work/stream.out" 0 99999999999999)
echo "package verify: live-reconfig stream total: requests=$1 max_latency=${2}ms"
echo "package verify: live-reconfig timeline join=$((t_join_done - t_join_start))ms increment=$((t_inc_done - t_inc_start))ms decrement=$((t_dec_done - t_dec_start))ms leave=$((t_leave_done - t_leave_start))ms"
